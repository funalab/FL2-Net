...
class ConvLSTM(nn.Module):
    ...
    Parameters:
        ...
        hidden_dim: Number of hidden channels(kernelの枚数)
    ...
    Example:
        >> x = torch.rand((64, 3, 32, 128, 128, 128))
        >> convlstm = ConvLSTM(64, 16, (3,3,3), 1, True, True, False)
        >> # model = ConvLSTM(input_dim=64, hidden_dim=16, kernel_size=(3,3,3), num_layers=1, batch_first=True, bias=True, return_all_layers=True)
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        # layer_output_list: lbtcdhw -> ltbcdhw
        if not self.batch_first:
            layer_output_list = layer_output_list.permute(0, 2, 1, 3, 4, 5, 6)
...
class ConvBLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        self.forward_net = ConvLSTM(input_dim, hidden_dim//2, kernel_size, num_layers,
                                    batch_first, bias, return_all_layers)
        self.reverse_net = ConvLSTM(input_dim, hidden_dim//2, kernel_size, num_layers,
                                    batch_first, bias, return_all_layers)
        self.batch_first = batch_first

    def forward(self, xforward):
        # xforward: T, B, C, D, H, W
        if self.batch_first:
            xreverse = torch.flip(xforward, dims=(1, ))
        else:
            xreverse = torch.flip(xforward, dims=(0, ))
        ...
        reversed_idx = list(reversed(range(yreverse.shape[0])))
        yreverse = yreverse[reversed_idx, ...]

        return torch.cat((yforward, yreverse), dim=-4)
...
class ConvGRUCell(nn.Module):
    ...
    def init_hidden(self, batch_size, image_size):
        depth, height, width = image_size
        return (Variable(torch.zeros(batch_size, self.hidden_dim, depth, height, width, device=self.conv_can.weight.device)))

    def forward(self, input_tensor, h_cur):
        """
        :param self:
        :param input_tensor: (b, c, d, h, w)
            input is actually the target_model
        :param h_cur: (b, c_hidden, d, h, w)
...
class ConvGRU(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        b, seq_len, c, d, h, w = input_tensor.size()
...
class ConvBGRU(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, num_layers,
                 batch_first=False, bias=True, return_all_layers=False):
        super().__init__()
        self.forward_net = ConvGRU(input_dim, hidden_dim//2, kernel_size, num_layers,
                                    batch_first, bias, return_all_layers)
        self.reverse_net = ConvGRU(input_dim, hidden_dim//2, kernel_size, num_layers,
                                    False, bias, return_all_layers)
        self.batch_first = batch_first
